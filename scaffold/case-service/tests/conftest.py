"""Test fixtures for case-service.

Two moving parts:

* case-service itself runs in-process (ASGITransport) against a dedicated
  ``case_db_test`` database.
* iam-service runs as a **subprocess** on a free port against its own dedicated
  ``identity_db_case_e2e`` database, so tests obtain real RS256 access tokens
  through the genuine login -> MFA -> verify flow. case-service's JWKS client is
  pointed at that spawned iam-service, so token signatures are verified for real.
...
_PG = {
    "host": os.getenv("CASE_DB_HOST", "localhost"),
    "port": os.getenv("CASE_DB_PORT", "5432"),
    "user": os.getenv("CASE_DB_USER", "pmp"),
    "password": os.getenv("CASE_DB_PASSWORD", "pmp_dev_only"),
}
_TEST_DB = "case_db_test"
_DSN = f"{_PG['user']}:{_PG['password']}@{_PG['host']}:{_PG['port']}/{_TEST_DB}"

os.environ["CASE_DATABASE_URL"] = f"postgresql+asyncpg://{_DSN}"
os.environ["CASE_DATABASE_URL_SYNC"] = f"postgresql+psycopg2://{_DSN}"

# --- spawned iam-service config --------------------------------------------
_IAM_DB = "identity_db_case_e2e"
...
_IAM_PORT = int(os.getenv("CASE_TEST_IAM_PORT", str(_free_port())))
...
_IAM_ENV = {
    **os.environ,
    ...
    "IAM_MFA_ENC_KEY": base64.urlsafe_b64encode(
        hashlib.sha256(b"case-e2e-mfa-key").digest()
    ).decode(),
    "IAM_ACCESS_TOKEN_TTL": "900",
}

# case-service verifies tokens against the spawned iam-service
os.environ["CASE_IAM_JWKS_URL"] = f"{_IAM_BASE}/api/v1/auth/jwks"
os.environ["CASE_JWT_ISSUER"] = "pmp-iam-service"
...
os.environ["EVENTS_TOPIC_PREFIX"] = f"cst_{uuid.uuid4().hex[:8]}."
...
engine = create_async_engine(os.environ["CASE_DATABASE_URL"], poolclass=NullPool)
...
_E2E_USERS = {
    "E2E-RW": "Patrol Officer",       # case.read + case.write
    "E2E-RO": "Station Commander",    # case.read, NOT case.write
    "E2E-NONE": "Auditor",            # neither
}
...
def token_rw(iam_server) -> str:
    return iam_access_token("E2E-RW")


@pytest.fixture(scope="session")
def token_ro(iam_server) -> str:
    return iam_access_token("E2E-RO")
...
@pytest.fixture
def auth_rw(token_rw) -> dict:
    return bearer(token_rw)


@pytest.fixture
def auth_ro(token_ro) -> dict:
    return bearer(token_ro)
...
@pytest_asyncio.fixture
async def make_case():
    """Insert a case row directly (there is no POST /cases in the contract)."""

    async def _make(status: str = "open", **overrides) -> Case:
        async with SessionLocal() as session:
            case = Case(
                case_number=overrides.get("case_number", f"CASE-{uuid.uuid4().hex[:8]}"),
                incident_id=overrides.get("incident_id"),
                status=status,
                lead_officer_id=overrides.get("lead_officer_id", uuid.uuid4()),
            )
            session.add(case)
