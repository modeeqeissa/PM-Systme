...
RBAC is enforced by validating the RS256 access token issued by iam-service:
the signature is checked against iam-service's JWKS (cached, see
``app/security/jwks.py``), the issuer and expiry are verified, and the required
permission code must appear in the token's ``permissions`` claim. Permission
codes match identity_db's ``permissions.code`` values (CLAUDE.md rule 4).
...
async def get_token_claims(
    ...
    """Verify the bearer token against iam-service's JWKS and return its claims."""
