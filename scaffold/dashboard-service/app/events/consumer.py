"""Kafka -> CQRS read models. At-least-once, idempotent on event_id (SRS §9.4)."""
import asyncio
import json
import logging
import uuid
...
from app.services.projections import apply_event

log = logging.getLogger("dashboard-service.consumer")
...
class DashboardConsumer:
    ...
    async def _handle(self, session: AsyncSession, envelope: dict) -> bool:
        ...
        await apply_event(session, envelope)
        return True

    async def process_available(self, timeout: float = 8.0) -> int:
        """Consume whatever is available; return the count of new events applied."""
        ...
        while loop.time() < deadline:
            ...
            if not batch:
                ...
                continue
            seen_any = True
            # Order the batch by occurred_at: many read models depend on a
            # prior event (CaseOpened before CaseStatusChanged, OfficerCreated
            # before an OfficerCertificationIssued, TransferRequested before
            # TransferStatusChanged) and those live on different topics, so
            # per-partition order alone isn't causal. Envelope timestamps are.
            envelopes = [
    ...
    async def run_forever(self) -> None:
        while not self._stopping.is_set():
            try:
                ...
                log.exception("dashboard consumer batch failed")
