...
Fetched from iam-service once and held in memory; re-fetched only when the cache
TTL expires or a token presents an unknown `kid`. No network call on the hot
path.
"""
import asyncio
import json
...
class JwksCache:
    ...
    async def _refresh(self) -> None:
        async with httpx.AsyncClient(timeout=5.0) as http:
            resp = await http.get(config.iam_jwks_url())
