"""DB-backed admin settings (integration_settings, Phase 5).

`INTEGRATION_GATEWAY_LOG_RETENTION_DAYS` is the seed/default; `PATCH
/integration-settings` writes a row that overrides it at runtime. The
...
_SPEC = {
    "log_retention_days": (int, lambda v: v >= 1, config.log_retention_days),
}
