"""FastAPI app factory for integration-gateway-service.

Phase 1 STUB: health check only. The integration_db schema (docs Section 9.3.9) is
migrated via Alembic but no domain endpoints exist yet. When the Phase 1 build
starts, register routers from app/routers/ below and (if the service publishes
or consumes events) add a lifespan like the built services.
"""
from fastapi import FastAPI

API_PREFIX = "/api/v1"


def create_app() -> FastAPI:
    app = FastAPI(
        title="PMP Integration Gateway Service",
        version="0.1-stub",
        description="Phase 1 stub — schema migrated, endpoints pending (FR section 4.9).",
    )

    # Phase 1: app.include_router(<resource>.router, prefix=API_PREFIX)

    @app.get("/health", tags=["ops"])
    async def health() -> dict[str, str]:
        return {"status": "ok", "service": "integration-gateway-service"}

    return app


app = create_app()
